...
float4 custom_main( in CustomShaderData data )
{
	...
	else
	{
		...
		float3 result = (basic_fresnel * 0.15) + (diffuse_color + data.ambient_lighting[0].xyz) * ao * data.base_material[0].xyz * mix(base_color.xyz, float3(0.0), metallic * roughness) + float3(rimLightIntensity * 0.0) + 0.5 * base_color.xyz + reflected_color * 4.0;
